...
extension HourMinuteSecondPicker
{
	...
	*/
	func setLeftImage(image: UIImage?,width:CGFloat=39)  {
	...
	*/
	func setRightImage(image: UIImage?,width:CGFloat=39)  {
		guard let img=image else {return}
		let right: UIImageView = UIImageView(frame: CGRect(x: 0.0, y: -10, width: self.frame.size.height, height: self.frame.size.height-5))
		right.image = img
